...
export function buildReadMaterialContentTool(
  ...
  return defineTool({
    ...
    label: '读取素材内容',
    description:
      '读取当前素材库其它阶段已保存的内容，每次调用只返回一个 stage_id（gimmick/character/pacing/intro/plot_refine/draft_excerpt）。不含编辑栏未写入的未保存草稿。',
    parameters: Type.Object({
      stage_id: Type.Union(
        ...
        {
          description:
            '素材阶段键名：gimmick=梗，character=人设，pacing=剧情设计，intro=导语设计，plot_refine=剧情细化，draft_excerpt=优秀正文片段；单次只读取该阶段',
        },
      ),
    }),
 ...
 * 所有素材阶段（梗/人设/剧情设计/导语设计/剧情细化/优秀正文片段）共用同一套工具：读取 + 写入。
 */
