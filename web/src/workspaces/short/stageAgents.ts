...
export type ShortWorkspaceStageAgentContext = {
  ...
  getDefaultWriteStageId?: () => ShortStageId
  stageBody: string
...
const WRITE_WORKSPACE_TARGET_STAGE_IDS: readonly ShortStageId[] = [
  'character_design',
  ...PLOT_CHILD_STAGES.map((stage) => stage.id),
  'outline',
]

const REPLACE_TARGET_STAGE_IDS: readonly ShortStageId[] = [
...
function isAllowedTargetStageId(
  ...
  return allowedStageIds.includes(id as ShortStageId)
}
...
const WRITE_TARGET_STAGE_ID_FIRST_NOTE =
  '调用 write_workspace_editor 时，如需写入非当前编辑框，必须先填写第一个参数 target_stage_id，再填写 text；不要先生成 text 后补 target_stage_id。可写入：character_design、plot_design、intro_design、plot_refine、outline。'
...
function targetStageIdSchema(allowedStageIds: readonly ShortStageId[]) {
  const description = allowedStageIds
    .map((id) => `${SHORT_STAGE_LABELS[id]}=${id}`)
    .join('、')
...
function plotChildStageIdSchema() {
  return Type.Union(
    ...
    {
      description:
        '剧情子方向：plot_design=剧情设计，intro_design=导语设计，plot_refine=剧情细化。',
    },
  )
}
...
export function buildSelectPlotChildStageTool(
  ...
  return defineTool({
    ...
    description:
      ...
      + '当用户在剧情智能体里要求处理剧情设计、导语设计或剧情细化中的另一个方向时，先调用本工具。'
    ...
    execute: async (_toolCallId, params) => {
      ...
      return textBlock(
        `已切换左侧选中到「${SHORT_STAGE_LABELS[targetStageId]}」。`,
      )
    },
  })
}
  ...
  return defineTool({
    ...
    parameters: Type.Object({
      source_stage_id: Type.Union(
        ...
        {
          description:
            '源阶段键名：人物（character_design）、剧情设计（plot_design）、导语设计（intro_design）、剧情细化（plot_refine）、大纲（outline）、正文编写（draft）、正文审阅（draft_review）、格式转换（format_conversion）',
        },
      ),
