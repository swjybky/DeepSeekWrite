          ...
          <section className="expert-draft-active-editor">
            ...
            <div className="expert-draft-editor-tabbar">
              ...
              {activeSectionTab === 'body' ? (
                <TextHistoryControls history={textHistory} historyKey={bodyKey} value={selectedSection.body} onChange={(body) => updateDraft((current) => ({ ...current, sections: updateSectionList(current.sections, selectedId, { body }) }))} disabled={draft.running} />
              ) : (
                <TextHistoryControls history={textHistory} historyKey={stateKey} value={selectedState.body} onChange={(body) => updateDraft((current) => ({ ...current, character_states: updateStateList(current.character_states, selectedId, { body }, selectedSection.title) }))} disabled={draft.running} />
              )}
            </div>
