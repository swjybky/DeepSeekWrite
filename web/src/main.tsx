...
function showBootFatalError(message: string) {
  ...
  rootEl.innerHTML = `
    <div class="boot-splash" role="alert">
      <p class="boot-splash-title">DeepWrite</p>
...
function boot() {
  window.addEventListener('error', (event) => {
    ...
    if (isBenignResizeObserverError(message)) {
      ...
      console.warn('[DeepWrite] 已忽略 ResizeObserver 布局通知:', message)
      return
    }
    if (appMounted) {
      event.preventDefault()
      console.error('[DeepWrite] 未处理脚本错误:', event.error ?? message)
      return
    }
    const detail =
  ...
  window.addEventListener('unhandledrejection', (event) => {
    ...
    if (isExpectedSendCancel) {
      event.preventDefault()
      console.warn('[DeepWrite] 用户操作已取消:', msg)
      return
    }
    if (
      ...
      event.preventDefault()
      console.warn('[DeepWrite] IndexedDB 瞬态错误（多窗口并发），已忽略:', msg)
      return
    }
    if (appMounted) {
      event.preventDefault()
      console.error('[DeepWrite] 未处理 Promise 错误:', reason)
      return
    }
    const detail =
