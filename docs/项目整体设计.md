...
> 本文基于当前源码梳理 Deep Write 的整体架构、目录职责、核心类和主要前端组件。若与旧版说明有差异，以当前源码为准。

## 1. 项目定位

Deep Write 是一款本地桌面写作应用，面向短篇小说、剧本和未来长篇创作。项目采用 Python 桌面壳 + React 静态前端 + 本地 JSON 存储 + Pi 智能体框架的混合架构。
...
`prompt_store.py` 管理提示词模板的读取、覆盖和迁移，并负责素材库、技能库的占位符渲染。书籍创作空间模板保持为原始系统提示词，不再渲染书名或分类占位符。
...
4. `.workspace_prompt_defaults_v2_reset` 首次执行时清除短篇/剧本提示词覆盖，使升级用户统一切换到新版五智能体默认提示词；其它偏好和用户数据不受影响。

主要渲染函数：

| 函数 | 作用 |
| --- | --- |
| `render_workspace_system_prompt` | 按阶段解析智能体模板并返回原始系统提示词。 |
...
| `app_data_root` | 当前用户的应用数据目录，如 macOS 的 `~/Library/Application Support/Deep Write`。 |
...
| `memoryMessageTransform.ts` | 每次请求前插入创作位置和长期记忆；普通阶段、正文总控、交互/后台分节共享同一格式。 |
...
| `DeepSeekWriteToolRenderer` | Pi 工具渲染器类，负责把 Deep Write 工具调用渲染成定制 UI。 |
...
| 平台 | 位置 |
| --- | --- |
| Windows | `%APPDATA%/Deep Write/.data` |
| macOS | `~/Library/Application Support/Deep Write/.data` |
| Linux | `${XDG_DATA_HOME:-~/.local/share}/Deep Write/.data` |
...
| 提示词层 | `prompt_store.py` 读取书籍系统提示词；素材/技能模板由后端或 `prompt/renderTemplate.ts` 渲染占位符。 |
...
5. 输出 onedir 目录 `dist/Deep Write/`。
...
Deep Write 的核心是“本地数据模型 + 阶段化工作台 + 可配置智能体工具”。Python 负责桌面壳、存储、安全的本地文件操作和提示词后端；React 负责工作台状态、编辑体验和 Pi 智能体运行；`bridge/` 是二者之间的稳定契约层。后续演进时，最重要的是保持阶段键、提示词路径、读取范围和工具写回目标四者一致。
