...
> 本文基于当前源码梳理 Write Claw（DeepseekWrite）的整体架构、目录职责、核心类和主要前端组件。若与旧版说明有差异，以当前源码为准。

## 1. 项目定位

Write Claw 是一款本地桌面写作应用，面向短篇小说、剧本和未来长篇创作。项目采用 Python 桌面壳 + React 静态前端 + 本地 JSON 存储 + Pi 智能体框架的混合架构。
...
3. 剧本提示词首次使用时，从短篇当前生效提示词复制一份独立覆盖。
...
| `app_data_root` | 当前用户的应用数据目录，如 macOS 的 `~/Library/Application Support/WriteClaw`。 |
...
| `writeClawToolRenderers.ts` | 自定义 Pi 工具调用渲染器，让工具状态在聊天 UI 中更清晰。 |
...
| `WriteClawToolRenderer` | Pi 工具渲染器类，负责把 Write Claw 工具调用渲染成定制 UI。 |
...
| `WriteClawToolRenderer` | `web/src/pi/writeClawToolRenderers.ts` | Pi 工具调用定制渲染器，让读写阶段、加载技能、替换文本等工具在聊天面板中有更清晰的展示。 |
...
| 平台 | 位置 |
| --- | --- |
| Windows | `%APPDATA%/WriteClaw/.data` |
| macOS | `~/Library/Application Support/WriteClaw/.data` |
| Linux | `${XDG_DATA_HOME:-~/.local/share}/WriteClaw/.data` |
...
4. 在 `writeClawToolRenderers.ts` 中补充工具渲染摘要。
...
Write Claw 的核心是“本地数据模型 + 阶段化工作台 + 可配置智能体工具”。Python 负责桌面壳、存储、安全的本地文件操作和提示词后端；React 负责工作台状态、编辑体验和 Pi 智能体运行；`bridge/` 是二者之间的稳定契约层。后续演进时，最重要的是保持阶段键、提示词路径、读取范围和工具写回目标四者一致。
