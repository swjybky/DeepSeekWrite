...
> **说明**：当前前端短篇分类选项见 `web/src/bridge.ts` 中 `SHORT_GENRE_OPTIONS`（含「世情」「现实情感」）。若后续改为「主类型」枚举而非多选分类，需同步修改本节判定逻辑。
...
- **B**：仅数据保存在 `.data/books.json`，不向磁盘写出章节文件，直至用户补设路径。
